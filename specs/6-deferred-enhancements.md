...
Entries marked **→ Designed (Mn)** have since been designed for an upcoming
milestone (see [5-milestones.md](5-milestones.md)); their v1 trade-off text
is kept for history.
...
| 2 | **True LangGraph `interrupt()` + checkpointer** → Designed (M7) | stdin prompt loop in the confidence gate | Resumable runtime adds complexity for a CLI. Restore when moving to a web UI or long-running/resumable runs. |
...
| 7 | **Resume (messy text) ingestion parity** → Designed (M8) | Skills JSON is the primary path; resume parsing best-effort | Structured seed data exists; messy-text parsing is its own problem. |
